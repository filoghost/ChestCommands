		...
		try {
			...
			for (int i = 0; i < icons.length; i++) {
				if (icons[i] != null) {
					...
					if (ChestCommands.hasPlaceholderAPI) {
						inventory.setItem(i, PlaceholderAPIBridge.replace(player,
								AttributeRemover.hideAttributes(icons[i].createItemstack(player))));
					} else {
					...
					if (extIcon.hasViewPermission() || extIcon.hasVariables()) {
						...
						if (extIcon.canViewIcon(player)) {
							...
							if (inventory.getItem(i) == null) {
								...
								if (ChestCommands.hasPlaceholderAPI) {
									inventory.setItem(i, PlaceholderAPIBridge.replace(player, newItem));
								} else {
								...
								if (ChestCommands.hasPlaceholderAPI) {
									meta.setDisplayName(PlaceholderAPIBridge.replace(player, extIcon.calculateName(player)));
									meta.setLore(PlaceholderAPIBridge.replace(player, extIcon.calculateLore(player)));
								} else {
